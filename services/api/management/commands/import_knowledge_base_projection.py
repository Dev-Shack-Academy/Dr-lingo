...
# Language codes supported by the Knowledge_Base_Projection dataset
SUPPORTED_LANGUAGES = {"zul": "isiZulu", "xho": "isiXhosa", "nso": "Sepedi", "sot": "SeSotho", "eng": "English"}


class Command(BaseCommand):
    help = "Import Knowledge Base Projection dataset from Hugging Face into RAG knowledge base"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help="Language code to import (zul, xho, nso, sot, eng)",
        )
        parser.add_argument(
            "--collection",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        # Auto-enable streaming when limit is set
        ...
        collection_name = options["collection"] or f"{lang_name} Knowledge Base"

        self.stdout.write(f"\n{'='*60}")
        self.stdout.write(self.style.HTTP_INFO(f"Importing {lang_name} ({lang_code}) Knowledge Base"))
        self.stdout.write(f"{'='*60}")
        self.stdout.write("  Dataset: sello-ralethe/Knowledge_Base_Projection")
        ...
        if ai_provider == "ollama":
            ...
            try:
                ...
                raise CommandError(
                    "The 'requests' library is required for Ollama connectivity check.\n"
        ...
        self.stdout.write(self.style.SUCCESS(f"\n✅ Import completed for {lang_name} Knowledge Base!"))
    ...
    def _import_dataset(self, collection, lang_code, split, streaming, limit, async_mode):
        ...
        self.stdout.write(f"  Language: {lang_code}")
        ...
        for attempt in range(max_retries):
            try:
                if streaming:
                    ...
                    ds = ds.filter(lambda x: x.get("language") == lang_code)
        ...
        for idx, item in enumerate(items_iterator):
            ...
            try:
                ...
                result = self._process_item(collection, item, idx, rag_service, async_mode, lang_code)
                if result == "created":
                    created_count += 1
                    embedding_success += 1
    ...
    def _process_item(self, collection, item, idx, rag_service, async_mode, lang_code):
        """Process a single dataset item and add to collection."""
        from api.models import CollectionItem

        # Extract text from the dataset item
        # The Knowledge_Base_Projection dataset has a "text" column
        ...
        # Build metadata
        metadata = {
            "source": "sello-ralethe/Knowledge_Base_Projection",
            "language": lang_code,
            "index": idx,
        }
